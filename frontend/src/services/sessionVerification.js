...
function classifierSignal(analysis = {}) {
  ...
  return {
    ...
    detail: classifier.document_type
      ? `Classifier returned ${classifier.document_type}.`
      : 'Classifier output was not returned.',
    trace: [
...
function buildPipeline(signals) {
...
export function buildVerificationModel(session = {}) {
  ...
  return {
    ...
    face: {
      similarity: Number.isFinite(Number(session.faceMatch)) ? Number(session.faceMatch) : null,
...
export function saveUploadSession(uploadSession = {}) {
  ...
  const fields = subjectFields(uploadSession.documentAnalysis)
  ...
  const pipeline = buildVerificationModel(baseSession).pipeline
  return sessionStore.addSession({ ...baseSession, pipeline })
}
...
export function recordSessionDecision(sessionId, decision, { reason = '', notes = '', actor } = {}) {
  ...
  useRBACStore.getState().addAuditEvent({
    type: next.type,
    actorId: actor?.id || session.officerId || 'unknown',
    targetId: sessionId,
    message: `${actor?.fullName || session.officerName || 'Officer'} ${next.label} ${sessionId}${reason ? ` (${reason})` : ''}`,
    metadata: { sessionId, decision, reason, notes: notes.trim() }
  })
}
